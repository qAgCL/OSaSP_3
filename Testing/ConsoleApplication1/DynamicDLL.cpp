...
int main()
{
	...
	HINSTANCE h;
	char das[] = "ChangeDLL.dll";

	h = LoadLibrary("ChangeDLL.dll");

	void(*DllFunc) (char* ProcName, char *OldStr, char* NewStr);
	DllFunc = (void(*) (char* ProcName, char *OldStr, char* NewStr))
		GetProcAddress(h, "find_change_string");
	DllFunc(ProgCharStr, OldCharStr, NewCharStr);
